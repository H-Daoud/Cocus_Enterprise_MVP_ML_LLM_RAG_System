...
"""
    ...
    status_dist = "\n".join(
        [
            f"  - `{k}`: {v} records"
            for k, v in analysis_results["field_profiles"]["status"]["value_distribution"].items()
        ]
    )
    country_dist = "\n".join(
        [
            f"  - `{k}`: {v} records"
            for k, v in analysis_results["field_profiles"]["country_code"][
                "value_distribution"
            ].items()
        ]
    )
    ...
    most_common_status = max(
        analysis_results["field_profiles"]["status"]["value_distribution"].items(),
        key=lambda x: x[1],
    )[0]
