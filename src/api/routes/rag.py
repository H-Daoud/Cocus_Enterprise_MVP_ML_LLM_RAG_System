...
async def query_rag(request: QueryRequest):
    ...
    try:
        ...
        context_text = "\n\n".join([f"Source [{i+1}]: {doc['content']}" for i, doc in enumerate(relevant_docs)])
...
"""
        ...
        if config.provider.value == "mock" or "Error" in response.answer:
            ...
            if "RESOURCE_EXHAUSTED" in response.answer:
                response.answer = "The system is currently in MOCK mode to ensure a smooth demo experience (Gemini API quota exceeded). " + \
