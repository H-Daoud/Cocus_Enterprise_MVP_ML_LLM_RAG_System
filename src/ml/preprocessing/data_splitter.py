...
class DataSplitter:
    ...
    def __init__(
        ...
        logger.info(
            f"DataSplitter initialized with ratios: "
            f"Train={train_ratio}, Train-Dev={train_dev_ratio}, "
            f"Dev={dev_ratio}, Test={test_ratio}"
        )

    def split(self, data: pd.DataFrame, stratify_column: str = None) -> Dict[str, pd.DataFrame]:
        """
        ...
        train_traindev, dev_test = train_test_split(
            data, train_size=train_traindev_size, random_state=self.random_state, stratify=stratify
        )
        ...
        splits = {"train": train, "train_dev": train_dev, "dev": dev, "test": test}
    ...
    def save_splits(
        self, splits: Dict[str, pd.DataFrame], output_dir: Path, file_format: str = "ndjson"
    ) -> Dict[str, Path]:
        """
        Save splits to disk
        ...
        metadata = {
            ...
            "splits": {
                ...
                for name, (df, path) in zip(
                    splits.keys(), zip(splits.values(), file_paths.values())
                )
            },
            "ratios": {
