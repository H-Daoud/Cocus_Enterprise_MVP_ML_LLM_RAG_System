        ...
        if target_id or target_email:
            ...
            try:
                ...
                for i, content in enumerate(all_docs["documents"]):
                    ...
                    doc_email = all_docs["metadatas"][i].get(
                        "customer_email"
                    )  # Assuming we added this to metadata

                    # Case insensitive match for ID
                    is_id_match = (
                        target_id and doc_id and target_id.group().upper() == doc_id.upper()
                    )
                    # Case insensitive match for Email
                    is_email_match = (
                        target_email
                        and doc_email
                        and target_email.group().lower() == doc_email.lower()
                    )
                    ...
                    if is_id_match or is_email_match:
                        ...
                        if current_id not in found_ids:
                            sources.append(
                                {
                                    "content": content,
                                    "document_id": all_docs["metadatas"][i].get(
                                        "source", "unknown"
                                    ),
